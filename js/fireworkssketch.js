let particles = [];
const gravity = .25;
const colors = ['red', 'orange', 'yellow', 'pink', 'cyan', 'magenta', 'white'];
let endColor;
let houses;

function preload(){
  font2 = loadFont('coast-Regular.otf');
}
...
function makeHouses() {
	...
	const houseWindowWidth = 15;
	const houseWindowHeight =35;
	for (let i = 0; i < houseCount; i++) {
		const houseHeight = random(35, 100);
		houses.fill(66,14,32);
...
function draw() {
	background(0);
   fill('hotpink');
    text('dont bring around a cloud \n to rain on my parade', 600,200);
	particles.forEach((p) => {
		p.step();
		p.draw();
	});
